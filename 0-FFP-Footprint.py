...
zm = float(17.575) # Measurement height above displacement height (i.e. z-d) [m] #Calculated from site data as 18.425m
z0 = float(2.75) # Roughness length [m] - enter [None] if not known #2.75 calculated as 0.1*27.5m 
umean = 0.0 # Mean wind speed at zm [ms-1] - enter [None] if not known #WS_1_1_1
h = float(1250.0) # Boundary layer height [m] #Assume 1250m boundary height between 1000 and 1500m
ol = 0.0 # Obukhov length [m] #Need to calculate in a function
sigmav = float(1.0) # Standard deviation of lateral velocity fluctuations [ms-1] #Assuming a sigmaV of 2 as between 1 and 3, google says around 2
ustar = 0.0 # Friction velocity [ms-1] #Column called USTAR_1_1_1
wind_dir = 0.0 # Wind direction in degrees (of 360) for rotation of the footprint #WD_1_1_1
...
flux_file = pd.read_csv("/Users/liambogucki/Desktop/Fluxcourse-Project/AMF_US-Syv_BASE-BADM_33-5/AMF_US-Syv_BASE_HH_33-5.csv", encoding="cp1252", skiprows=2)
    ...
    #Running the model generation
        try:
                FFP = myfootprint.FFP(
                        ...
                        h=h,
                        ol=float(calcObhukov(airDensity(row.TA_1_1_1), row.USTAR_1_1_1, row.TA_1_1_1, row.H_1_1_1)),
                        sigmav=sigmav,
